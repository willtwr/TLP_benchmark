...
def get_graph(scoreList, fignum, evalType, testname):
    ...
    for i in range(len(rankList)):
        ...
        if len(attr.successRateList) == len(thresholdSetOverlap):
            if i < MAXIMUM_LINES:
                ...
                print(attr.tracker)
                print("aveFPS: " + str(attr.fps))
                print("SuccessRate (t=0.5): " + str(attr.successRateList[10]))
                plt.plot(thresholdSetOverlap, attr.successRateList, 
                    c = LINE_COLORS[i], label='{0} [{1:.1f}]'.format(tracker, ave), lw=2.0, ls = ls)
            else:
                plt.plot(thresholdSetOverlap, attr.successRateList, 
    ...
    plt.title('Success plot of OPE - TLP')
    ...
    plt.savefig(BENCHMARK_SRC + '{0}_sq.png'.format(evalType), dpi=74, bbox_inches='tight')
    plt.show()  
    return plt
